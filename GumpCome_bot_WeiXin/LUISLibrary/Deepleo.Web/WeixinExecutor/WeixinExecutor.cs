            ...
            switch (message.Type)
            {
                case WeixinMessageType.Text://文字消息
                    {
                        ...
                        string BotMessage1 = null;

                        //本地调用LUIS
                        LUISResult luis = await LUISHelper.MakeRequest(userMessage);
                        //BotMessage1 = VendingLuisHelper.GetAnswer(luis.LUISIntent.intent, luis);
                        BotMessage1 = LifeAssistance.GetAnswer(luis.LUISIntent.intent, luis);
                        //BotMessage1 = "欢迎使用，您输入了：" + userMessage + " 要不你发个图片试试？ http://i1.piimg.com/567571/4c20a0e8d637b8a0.jpg";
                        /*
                        if (userMessage.ToLower().Contains("bot"))
                        {
                            string name = System.Web.Configuration.WebConfigurationManager.AppSettings["directline"];
                            if (name == "true")
                                BotMessage1 = "DirectLine";
                            else
                                BotMessage1 = "LUIS RESTFUL API";
                        }
                        else if (userMessage.Contains("切换"))
                        {
                            string name = System.Web.Configuration.WebConfigurationManager.AppSettings["directline"];
                            if (name == "true")
                            {
                                System.Web.Configuration.WebConfigurationManager.AppSettings.Set("directline", "false");
                                MvcApplication.CleanBot();
                                BotMessage1 = "已切换成 LUIS RESTFUL API 模式";
                            }
                            else
                            {
                                System.Web.Configuration.WebConfigurationManager.AppSettings.Set("directline", "true");
                                MvcApplication.CleanBot();
                                BotMessage1 = "已切换成 BOT Direct Line 模式";
                            }
                        }
                        else if (userMessage.Contains("几点了") || userMessage.Contains("几点啦"))
                        {
                            DateTime currenttime = DateTime.Now.AddHours(8);
                            BotMessage1 = "现在是北京时间：" + currenttime.ToString("t") + " " + currenttime.ToString("dddd, MMMM dd yyyy");
                        }
                        else
                        {
                            try
                            {
                                if (MvcApplication.IsBotMode == "true")
                                {
                                    BotMessage1 = await MSBot.PostMessage(userMessage, openId);
                                }
                                else
                                {
                                    //本地调用LUIS
                                    LUISResult luis = await LUISHelper.MakeRequest(userMessage);
                                    BotMessage1 = VendingLuisHelper.GetAnswer(luis.LUISIntent.intent, luis);
                                    //BotMessage1 = "欢迎使用，您输入了：" + userMessage + " 要不你发个图片试试？";
                                }
                            }
                            catch (Exception ex)
                            {
                                BotMessage1 = ex.Message;
                            }
                        }
                        */
                        result = ReplayPassiveMessageAPI.RepayText(openId, myUserName, BotMessage1);
                    }
                    break;
                ...
                case WeixinMessageType.Voice://语音消息
                    #region 语音消息
                    {
                        ...
                        LUISResult luis = await LUISHelper.MakeRequest(messageRec);

                        voiceMessage = VendingLuisHelper.GetAnswer(luis.LUISIntent.intent, luis);

                        result = ReplayPassiveMessageAPI.RepayText(openId, myUserName, string.Format("语音消息:{0}", voiceMessage));
                        ...
                        //result = ReplayPassiveMessageAPI.RepayText(openId, myUserName, messageRec);
                    }
                    #endregion
                    break;
